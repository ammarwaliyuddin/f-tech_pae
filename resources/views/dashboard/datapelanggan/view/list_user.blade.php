    ...
    <table class="table table-report -mt-2" id="myTabel">
        <thead>
            <tr>
                ...
                <th class="text-center whitespace-nowrap">Id Level</th>
                <th class="text-center whitespace-nowrap">Alamat</th>
                <th class="text-center whitespace-nowrap">HP</th>
                <th class="text-center whitespace-nowrap">Id Kota</th>
                <th class="text-center whitespace-nowrap">Id Kecamatan</th>
        ...
        <tbody>
            @php
                ...
                <tr class="intro-x">
                    ...
                    <td
                        class="text-center">{{ $item->id_level }}
                    </td>
                    <td
                    ...
                    <td
                        class="text-center">{{ $item->id_kota }}
                    </td>
                    <td
                        class="text-center">{{ $item->id_kecamatan }}
                    </td>
...
<!-- END: Data List -->
            ...
            <a class="flex items-center mr-3 btn btn-sm" id="btn-edit" data-id_user="{{ $item->id_user }}" data-nama_user="{{ $item->nama_user }}" data-email="{{ $item->email }}" data-password="{{ $item->password }}" data-level="{{ $item->level }}" data-alamat="{{ $item->alamat }}" data-hp="{{ $item->hp }}" data-id_kota="{{ $item->id_kota }}" data-kecamatan="{{ $item->kecamatan }}"> <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-square w-4 h-4 mr-1"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg> Edit </a>
